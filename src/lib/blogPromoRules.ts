 ...
 * 첫 화면 이벤트 팝업을 띄울지.
 *
 * 지금은 꺼 둔다 — 조건을 다 갖춘 글이 실제로 통과해 지급되는 것까지 눈으로 확인한 뒤에
 * 사람들을 부르는 게 맞다. 안내부터 띄우고 지급이 안 되면 첫인상이 그걸로 끝난다.
 * 켤 때는 이 값만 true 로 바꾸면 된다(팝업 자체는 이미 검사로 고정돼 있다).
 */
export const BLOG_EVENT_POPUP_ENABLED = false
