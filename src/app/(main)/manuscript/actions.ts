...
2. 한국실용글쓰기 답안 기준 부합 (40점)
   ...
   - **글자 수는 직접 세지 않는다.** [분량] 줄에 적힌 수를 그대로 쓴다 — 언어 모형은 글자를
     정확히 못 세므로, 직접 센 수로 분량을 지적하면 사실과 다른 감점이 된다(2026-09-10 문의).
...
export async function gradeManuscript(text: string, topic: string): Promise<GradeResult | GradingError> {
  ...
  // text·topic은 사용자가 넣은 값이 그대로 유료 API로 들어간다 → 길이가 곧 비용이므로 상한을 건다.
  const tooLong = gradingLimitError(text, MAX_ANSWER_CHARS, '작성한 글') ?? gradingLimitError(topic, MAX_TOPIC_CHARS, '주제')
  ...
  try {
    ...
    response = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        // 글자 수는 우리가 세어 준다 — 모형이 직접 세면 틀린 수로 감점한다(2026-09-10 문의).
        content: `주제: ${topic}\n[분량] 작성한 글 ${Array.from(String(text ?? '')).filter(c => c !== '\n').length}자(띄어쓰기 포함, 줄바꿈 제외)\n\n작성한 글 (원고지 형식, 줄바꿈은 \\n으로 표시):\n${text}`,
      }],
    })
  } catch (err) {
