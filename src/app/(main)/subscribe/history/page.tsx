    ...
    <div className="animate-fade-up max-w-2xl mx-auto">
      ...
      {subs.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#e2e8f0]">
          ...
          <Link href="/subscribe" className="mt-4 btn-gold inline-flex items-center gap-1.5 font-bold px-5 py-3.5 rounded-xl text-sm">
            이용권 보기
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
          {subs.map(s => {
            ...
            const badge = active
              ...
              : cancelled
                ? { icon: <XCircle className="h-3.5 w-3.5" />, text: '해지됨', cls: 'bg-red-50 text-red-700' }
                : expired
