    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">총 {questions?.length ?? 0}문항</p>
        </div>
        <Link
      ...
      {groups.size === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 text-sm">등록된 문제가 없습니다.</p>
          <Link href="/admin/questions/new" className="inline-block mt-3 text-sm text-[#1e3a5f] font-medium hover:underline">
            첫 번째 문제 추가하기 →
          </Link>
        </div>
      ) : (
            ...
            return (
              <div key={key} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="bg-gray-50 px-5 py-3 border-b border-gray-200 flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-500">{qs!.length}문항</span>
                </div>
                <div className="divide-y divide-gray-100">
                  {qs!.map(q => (
                    <div key={q.id} className="flex items-center gap-3 px-5 py-3.5">
                      <span className="text-xs font-bold text-gray-400 w-6 shrink-0">{q.number}번</span>
