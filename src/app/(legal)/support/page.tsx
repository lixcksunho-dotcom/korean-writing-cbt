...
const FAQS = [
  ...
  {
    q: `유료인데 AI 첨삭이 하루 ${DAILY_GRADE_LIMIT}회에서 막혔어요`,
    a: `이용권 기간 중 횟수 제한은 없지만, 계정 공유와 자동화·대량 이용을 막기 위해 하루 ${DAILY_GRADE_LIMIT}회까지만 받을 수 있어요(서술형 9문항 기준 모의고사 ${Math.floor(DAILY_GRADE_LIMIT / 9)}회 분량). 한국 시간 자정에 다시 열립니다. 궁금한 점은 고객센터로 문의해 주세요.`,
  },
  {
    q: '회원 탈퇴는 어떻게 하나요?',
    a: '로그인 후 오른쪽 위 [계정] → [회원 탈퇴]에서 바로 할 수 있어요. 로그인 정보와 학습 기록은 즉시 지워지고, 결제 기록만 법령에 따라 5년간 보관합니다(누구의 것인지는 끊어 둡니다). 남은 이용권은 사라지니 환불 대상이면 먼저 환불을 신청해 주세요.',
  },
  {
    q: '계정을 여러 명이 같이 써도 되나요?',
    a: `아니요. 이용권은 결제한 본인 1인만 사용할 수 있고, 한 계정은 최대 ${DEVICE_LIMIT}대 기기까지만 이용 가능합니다. 계정 공유나 기기 수 초과 등 부정 이용이 확인되면 사전 통지 없이 이용이 제한될 수 있으며, 이 경우 잔여 기간은 환불되지 않습니다.`,
  },
]
