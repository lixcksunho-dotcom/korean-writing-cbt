...
call claude -p "CLAUDE.md의 워커 규칙에 따라 진행해. 보고는 간결하게 — 무엇을 왜 했는지와 검증 결과만." --model fable --allowedTools "Read,Edit,Write,Bash" >> logs\worker.log 2>&1
...
if errorlevel 3 (
  call claude -p "CLAUDE.md의 워커 규칙에 따라 진행해. 보고는 간결하게 — 무엇을 왜 했는지와 검증 결과만." --model sonnet --allowedTools "Read,Edit,Write,Bash" >> logs\worker.log 2>&1
)
