...
| `npm run check:alert-triage` | 사고 목록에서 '볼 것'만 위로 오는가 — 검사 자국·기계가 끝낸 일(행사 발급, 자동 통과 신청, 사후 확인 회수, 회선 끊김 오류)·처리 끝난 문의가 내려가는가 | 12건 중 9건이 검사 자국이었고, 그걸 걷어낸 뒤에도 11건이 전부 기계가 끝낸 일이었다(2026-09-11). 가짜가 목록을 채우면 사람은 목록을 안 읽고, 그때 진짜 하나가 묻힌다 — 결제 실패 1건을 16일간 몰랐다 |
...
**카카오페이 채널에서 건드리면 안 되는 파라미터**(포트원 연동 문서 기준):

| 파라미터 | 규칙 |
| --- | --- |
| `windowType` | PC는 IFRAME, 모바일은 REDIRECTION만 된다. **비워 두면 알아서 그렇게 잡힌다** — 다른 값을 주면 에러 |
| `currency` | 원화만. `CURRENCY_KRW` 그대로 |
| `locale` | KO_KR만 지원(주지 않는다) |
| `easyPayProvider` | 카카오페이는 PG 자체가 간편결제사라 무시된다(넣어도 무해) |
| `isEscrow`·`isCulturalExpense`·`productType` | 지원 안 함 — 넣어도 포트원 안에만 저장된다 |

**채널이 열리면 확인할 것**: 지금은 모든 결제 수단에 휴대폰 번호를 필수로 받는데, 그건
KG이니시스 일반결제 요구사항이다. 카카오페이도 필요한지는 실제로 열어 봐야 안다 —
필요 없다면 카카오페이에서는 안 받는 편이 좋다(입력 한 칸이 곧 이탈이다).
