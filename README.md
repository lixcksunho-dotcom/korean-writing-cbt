...
| `npm run check:ui-authed` | 로그인 뒤 15개 화면 × 2모드의 명암비·휴대폰 사용성 | 위 두 검사는 공개 면만 본다. 정작 돈 낸 사람이 오래 머무는 곳은 로그인 뒤다 |
...
느린 회선(1.6Mbps·150ms·CPU 4배)에서 
> korean-writing-cbt@0.1.0 check:vitals
> node scripts/web_vitals_check.mjs


체감 성능 — 휴대폰 · 느린 회선(1.6Mbps/150ms) · CPU 4배 느리게

면                TTFB     FCP     LCP      CLS     TBT      반응
/                381ms   2436ms   2436ms   0.0019   1279ms     40ms   ←
/spelling        151ms   2444ms   2444ms   0.0000   1638ms     16ms   ←
/blog            106ms   2172ms   2172ms   0.0000   1414ms     32ms   ←
/subscribe       148ms   1792ms   2288ms   0.0000    529ms     32ms 
/exam-info       156ms   2332ms   2332ms   0.0000   1091ms     32ms   ←

기준 초과 4건
  / TBT 1279ms (기준 600)
  /spelling TBT 1638ms (기준 600)
  /blog TBT 1414ms (기준 600)
  /exam-info TBT 1091ms (기준 600)로 잰 값이다.
...
- **TBT 1100~1800ms** — 기준(600ms)을 넘는다. 하이드레이션이 한 덩어리(2.7초짜리 긴 작업
  하나)로 돌면서 초기에 메인 스레드를 막는다. JS 244KB 중 대부분이 공용 번들이다.

의 첫 화면 밖 클라이언트 조각(맛보기 문제·하단 고정 CTA)을 으로
떼 보는 실험을 했는데 **효과가 없었다**(5회 중앙값 1744ms, 대조군 /blog도 같이 흔들림).
남은 비용은 개별 컴포넌트가 아니라 공용 번들이라, 줄이려면 클라이언트 컴포넌트 수를
구조적으로 줄여야 한다. 같은 실험을 반복하지 않도록 적어 둔다.

측정 자체가 4배 스로틀 + 이 개발 기기 부하에 흔들린다(같은 면을 5번 재면 1475~2250ms).
