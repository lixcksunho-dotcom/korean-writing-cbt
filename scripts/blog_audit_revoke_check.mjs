...
// 4) 받기 전에 회수 조건을 알려 주는가.
//    사후 확인은 글이 내려가거나 광고 표시가 사라지면 이용권을 멈춘다. 그 조건을
//    미리 안 알리고 나중에 끊으면, 몰랐던 사람에게 말없이 뺏는 셈이 된다.
{
  ...
  const terms = [
    ...
    ['30일은 두어 달라는 부탁', /RECOMMENDED_KEEP_DAYS}일은 그대로 둬/],
    ['그 30일이 의무가 아니라는 말', /의무는 아니에요/],
    ['내용을 강요하지 않는다는 말', /좋게 써 달라고 부탁드리지 않아요/],
  ]
  for (const [label, re] of terms) {
    if (re.test(form)) ok(`받기 전에 ${label}이 있다`)
    else bad(`고지 누락 — ${label}`, '조건을 모른 채 참여하게 된다')
  }
}
