...
// 화면 없이도 볼 수 있는 것은 여기서 미리 본다.
{
  ...
  if (/멈춰|꺼집니다|멈춥니다|못 씁니다/.test(src)) ok('내리면 못 쓰게 된다고 밝힌다')
  else bad('회수 고지', '없다')
  if (/되살아/.test(src)) ok('다시 올리면 되살아난다고 함께 밝힌다')
...
try {
  ...
  if (!heldBack) {
    ...
    if (/내리면[\s\S]*(못 쓰|못 씁|멈)/.test(text) && /되살아/.test(text)) ok('내리면 꺼지고 다시 올리면 되살아난다고 미리 알린다')
