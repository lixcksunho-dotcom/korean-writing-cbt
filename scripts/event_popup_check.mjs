...
// 화면 없이도 볼 수 있는 것은 여기서 미리 본다.
{
  ...
  if (/회수하고|회수됩니다|못 씁니다/.test(src)) ok('내리면 회수된다고 밝힌다')
  else bad('회수 고지', '없다')
  if (/다시 신청하실 수 없어요/.test(src)) ok('다시 신청할 수 없다는 것까지 밝힌다')
  else bad('한쪽만 고지', '회수만 말하고 재신청이 막힌다는 말이 없다')
...
try {
  ...
  if (!heldBack) {
    ...
    if (/내리면[\s\S]*회수/.test(text) && /다시 신청하실 수 없어요/.test(text)) ok('내리면 회수되고 다시 신청도 못 한다고 미리 알린다')
    else bad('회수 고지', text.slice(0, 120).replace(/\n/g, ' | '))
    if (/30일은 그대로 둬 주세요/.test(text) && /의무는 아니/.test(text)) ok('30일은 두어 달라고 부탁한다 — 조건이 아니라는 말과 함께')
    else bad('30일 권고', text.slice(0, 160).replace(/\n/g, ' | '))
