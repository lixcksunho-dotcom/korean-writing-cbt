...
for (const path of PAGES) {
  ...
  await page.waitForTimeout(9000) // 느린 회선에서 한글 서브셋이 늦게 도착하는 것까지 본다
  ...
  rows.push({ path, ...v, lcp: Math.round(v.lcp) })
  await ctx.close()
}
await browser.close()
...
console.log('면'.padEnd(14), 'TTFB'.padStart(6), 'FCP'.padStart(7), 'LCP'.padStart(7), 'CLS'.padStart(8))
for (const r of rows) {
  const flag = (r.lcp > 2500 || r.cls > 0.1) ? '  ←' : ''
  console.log(r.path.padEnd(14), String(r.ttfb).padStart(5) + 'ms', String(r.fcp).padStart(6) + 'ms', String(r.lcp).padStart(6) + 'ms', r.cls.toFixed(4).padStart(8), flag)
