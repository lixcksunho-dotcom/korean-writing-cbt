...
- **막힌 단계**: 실제 발행(`upload_post.py posts/silgeul-review-0908`)은 자동 모드 분류기가 외부 공개 행동으로 차단. 사람이 그 한 줄을 실행해야 한다.
- **발행 뒤 이어서 할 것**(스크립트 준비 완료 `scripts/blog_event_live_run.mjs`): `submit <글주소>` 로 신청·자동 심사·7일 이용권 지급 확인 → `audit` 로 사후 감사 정상 확인 → 글을 내린 뒤 `audit` 재실행으로 회수 확인 → `cleanup` 으로 검사 계정·지급 행 삭제.
